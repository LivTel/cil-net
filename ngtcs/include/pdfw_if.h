...
int pd_find_devices(void);

// Cleanup boards after use
int pd_clean_devices(void);
...
ssize_t pd_aout_write_scan(
   int board,
   const char *buffer,
   size_t count);  // function for write() from aout
...
ssize_t pd_din_read(int board, char* buffer, size_t count);
ssize_t pd_dout_write(int board, const char* buffer, size_t count);
