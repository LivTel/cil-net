...
public class CilResponseParser {
	...
	protected void skipWhiteSpace() {
		while (parsePosition < parseLength && (args.charAt(parsePosition) == ' ')) {
			parsePosition++;
		}
	}
	...
	protected void readNumber() throws NumberFormatException {
		while (parsePosition < parseLength
				&& (Character.isDigit(args.charAt(parsePosition)) || (args.charAt(parsePosition) == 'X'))) {
	...
	protected void readChar(char c) throws ParseException {
		if (parsePosition == parseLength || (args.charAt(parsePosition) != c))
			throw new ParseException("Illegal char (readChar) [" + args.charAt(parsePosition) + "] expected [" + c
					+ "]", parsePosition);
		parsePosition++;
